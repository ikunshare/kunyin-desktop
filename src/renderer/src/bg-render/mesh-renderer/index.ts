...
import { blurImageToroidal } from '../img'
...
class Mesh implements Disposable {
  ...
  draw() {
    ...
    if (this.wireFrame) {
      ...
      gl.drawElements(gl.TRIANGLES, this.vertexIndexLength, gl.UNSIGNED_SHORT, 0)
    }
  }
  ...
  resize(vertexWidth: number, vertexHeight: number): void {
    ...
    const vertexData = new Float32Array(vertexWidth * vertexHeight * (2 + 3 + 2))
...
class BHPMesh extends Mesh {
  ...
  constructor(gl: RenderingContext, attrPos: number, attrColor: number, attrUV: number) {
...
class GLTexture implements Disposable {
  ...
  constructor(
    ...
    // REPEAT + 环形模糊后的四方连续纹理：旋转采样越出 [0,1] 时依旧无缝。
    // 不能用 MIRRORED_REPEAT——镜像折返线上颜色梯度反向，会在画面上留下
    // 一条随旋转缓慢移动的「折痕弧线」（纹理是 32x32 POT，REPEAT 在 WebGL1 合法）
...
export class MeshGradientRenderer extends BaseRenderer {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    // 上游此处探测 EXT_color_buffer_float / OES_texture_float 等浮点纹理扩展并打 warn，
    // 但本渲染器的封面纹理与 FBO 一律是 RGBA/UNSIGNED_BYTE（上游的 supportTextureFloat
    // 分支早已弃用），且 EXT_color_buffer_float 属 WebGL2、在 webgl1 上下文必然为 null，
    // 探测只会每次启动刷无意义的告警，故移除。
    ...
    this.mainProgram = new GLProgram(gl, meshVertShader, meshFragShader, 'main-program-mg')
  ...
  override async setAlbum(
    ...
    // 环形模糊：使纹理四方连续（配合 GLTexture 的 REPEAT 采样，见上）
    blurImageToroidal(imageData, 2, 4)
