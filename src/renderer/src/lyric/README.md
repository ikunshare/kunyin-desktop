...
倍速播放支持（2026-09-21）——上游引擎没有速率概念，宿主开倍速后歌词按 1× 走，越放越落后：

| 位置                                                                                                        | 改动                                                                                                                         | 原因                                                                                                                                                                              |
| ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `player/base/core/index.ts`                                                                                 | `state.rate`（默认 1）；`handleGetCurrentTime()` 的流逝时间乘以它；新增 `setPlaybackRate()`（先按旧速率把 `time.seek` 归位再换速率，然后发事件）与 `currentPlaybackRate` | 引擎的时钟是 `seek + (performance.now() - start)`，纯墙钟。倍速下宿主每墙钟秒消耗的内容是它的 `rate` 倍，不乘就漏算                                                              |
| `player/base/interface.ts`                                                                                  | 新增事件 `playbackRateUpdate`                                                                                                  | DOM 层要据此重驱在飞的动画                                                                                                                                                        |
| `player/dom/components/context.ts`                                                                          | `ComponentContext` 新增可变字段 `playbackRate`（默认 1）                                                                       | 逐字动画拿不到 `BaseLyricPlayer`，只有 `ComponentContext`；速率从这里下发                                                                                                          |
| `player/dom/core/index.ts`                                                                                  | 构造时镜像 `player.currentPlaybackRate`；订阅 `playbackRateUpdate` → 写回 context 并 `scheduleLayoutUpdate({ isSeek: true })` | `isSeek` 才会让 `layout.update` 对**没换行**的当前行也重新 `play`/`pause`——那是唯一会重设 `animation.playbackRate` 的路径。擦除动画按整行建，不强制重驱的话换速率要等下一行才生效 |
| `.../syllable/animation/mask.ts`、`float.ts`、`emphasize.ts`、`line/interlude/dot.ts`                        | 原先硬编码的 `animation.playbackRate = 1` 改读 `context.playbackRate`（`dot.ts` 原本没设过，补上）                            | 这些是 WAAPI 墙钟动画，而行时钟已经按速率走；不同步就会「行换得快、字擦得慢」                                                                                                      |
| `.../syllable/animation/mask.ts` + 调用方 `word.ts` / `annotation/base.ts`                                  | `MaskAnimation` 构造函数新增第三个参数 `context`                                                                              | 内层 `MaskAnimation` 原本只拿到 `host` 与 `lineDuration`，够不到速率                                                                                                               |

不变的两处：`emphasize.ts` 的 `disableRate`（换行时收尾用的快进倍率）是瞬时收尾、与内容时序无关；
行滚动的 CSS transition 同理，都保持原速。
